...
class CommentsService {
	static async fetchComments(userUID: string): Promise<Comment[]> {
		try {
			const {data, error} = await supabase.from("comments").select("*").eq("userUID", userUID).order("created_at", {ascending: false});
